    ...
    <div className="relative min-h-screen bg-white text-black overflow-x-hidden flex flex-col font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10 p-6 md:p-10 relative">
        ...
        <div
          ...
          className="relative w-full aspect-[4/5] bg-white border border-black rounded-[2rem] overflow-hidden shadow-md cursor-pointer group mb-10"
        >
          <div className="absolute inset-0 bg-neutral-100 flex items-center justify-center p-2">
            <img
              src="https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?auto=format&fit=crop&q=80&w=800"
              alt="Future Editorial Model"
              className="w-full h-full object-cover grayscale brightness-95 group-hover:scale-105 transition-transform duration-700"
              referrerPolicy="no-referrer"
            />
          </div>
          <div className="absolute top-0 left-0 w-2 h-20 bg-[#D4FF00]"></div>
          <div className="absolute bottom-8 left-8 z-10 flex flex-col gap-1 text-black">
            <span className="text-[10px] font-mono font-black tracking-widest text-[#D4FF00] drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">01 // 04</span>
            <div className="w-24 h-0.5 bg-black/40 relative overflow-hidden">
              <div className="absolute top-0 left-0 h-full w-[45%] bg-[#D4FF00]"></div>
            </div>
            <span className="text-[8px] font-black tracking-widest uppercase text-white drop-shadow-[0_1.5px_3px_rgba(0,0,0,0.9)] mt-1 font-space">EDITORIAL NODE</span>
          </div>
          <div className="absolute bottom-6 right-6 w-14 h-14 bg-[#D4FF00] rounded-full border border-black flex items-center justify-center shadow-lg transform group-hover:scale-110 active:scale-95 transition-transform">
            <span className="material-icons-round text-black text-xl animate-pulse">north_east</span>
          </div>
        </div>
